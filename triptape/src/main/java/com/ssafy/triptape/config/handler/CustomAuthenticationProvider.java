package com.ssafy.triptape.config.handler;
...
//        UserDto userDetailsDto = (UserDto) userDetailsService.loadUserByUsername(userId);

        UserDto userDto;
        if (!(userDto.getPw().equalsIgnoreCase(userPw) && userDto.getPw().equalsIgnoreCase(userPw))) {
            throw new BadCredentialsException(userDto.getName() + "Invalid password");
        }
